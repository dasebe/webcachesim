...
public:
    ...
    GDBTMeta(const uint64_t & key, const uint64_t & size, const uint64_t & past_timestamp,
        ...
        _past_distances = vector<uint64_t >(GDBT::max_n_past_timestamps);
...
public:
    ...
    void append(const GDBTPendingTrainingData& pending, float & future_distance) {
        ...
        for (int i = 0; i < pending.past_distances.size(); ++i) {
            indices.emplace_back(i);
            data.emplace_back(pending.past_distances[i]);
        }
        counter += pending.past_distances.size();
        ...
        data.push_back(pending.past_distances.size());
        ++counter;
