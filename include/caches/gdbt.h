...
public:
    ...
    static vector<uint32_t > edwt_windows;

    static vector<double > hash_edwt;
    static uint32_t max_hash_edwt_idx;
    ...
    inline void update(const uint64_t &past_timestamp, const uint64_t &future_timestamp) {
        ...
        for (uint8_t i = 0; i < n_edwt_feature; ++i) {
            uint32_t _distance_idx = min(uint32_t (_distance/edwt_windows[i]), max_hash_edwt_idx);
            _edwt[i] = _edwt[i] * hash_edwt[_distance_idx] + 1;
        }
    }
};
...
vector<uint32_t > GDBTMeta::edwt_windows = vector<uint32_t >(GDBTMeta::n_edwt_feature);
vector<double > GDBTMeta::hash_edwt = vector<double >();
uint32_t GDBTMeta::max_hash_edwt_idx = 0;
    ...
    void init_with_params(map<string, string> params) override {
        ...
        for (auto& it: params) {
            if (it.first == "sample_rate") {
                ...
                n_edwt_feature = stoull(it.second);
        ...
        GDBTMeta::n_edwt_feature = n_edwt_feature;
        GDBTMeta::edwt_windows = vector<uint32_t >(GDBTMeta::n_edwt_feature);
        for (uint8_t i = 0; i < GDBTMeta::n_edwt_feature; ++i) {
            GDBTMeta::edwt_windows[i] = pow(2, GDBTMeta::base_edwt_window+i);
        }
        GDBTMeta::max_hash_edwt_idx = (uint64_t) (threshold/pow(2, GDBTMeta::base_edwt_window))-1;
        GDBTMeta::hash_edwt = vector<double >(GDBTMeta::max_hash_edwt_idx+1);
        for (int i = 0; i < GDBTMeta::hash_edwt.size(); ++i)
            GDBTMeta::hash_edwt[i] = pow(0.5, i);
